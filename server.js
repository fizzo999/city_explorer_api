...
function handleGetRestaurants(req, res) {
  ...
  const url7 = `https://api.yelp.com/v3/businesses/search?term=restaurant&limit=5&latitude=${req.query.latitude}&longitude=${req.query.longitude}&offset=${offset}`;
  superagent.get(url7)
    .set('authorization', `bearer ${YELP_API_KEY}`)
    .then((output) => {
      console.log('here is THE OUTPUT', output.body.businesses);
      const yelpArray = output.body.businesses.map((singleRestaurant) => new Restaurant(singleRestaurant));
      res.send(yelpArray);
    })
    .catch(errorThatComesBack => {
      console.log(errorThatComesBack);
      res.status(500).send('Sorry something went wrong with them Restaurants !!!!');
    });
}
...
function handleMovieRequests(req, res) {
  console.log('here is this HUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUGHE REQUEST OBJECT', req);
